...
const gpioMap_t  keys_config[] = { TEC1 , TEC2 , TEC3 , TEC4 } ;
...
static void keys_event_handler_button_release( uint32_t index )
{
    ...
    last_event.value = keys_data[index].time_up - keys_data[index].time_down;
    last_event.index = keys_config[index];
    last_event.type = EVENT_KEY_PRESSED;
    xQueueSend(cola_1, &last_event, portMAX_DELAY);
    xQueueSend(cola_2, &last_event, portMAX_DELAY);
    taskEXIT_CRITICAL();
}
